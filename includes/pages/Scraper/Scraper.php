<?
//$customers = getBrickData("scraper",false,"*",false,false,$connection);
...
createAttribute($brickname,$attributesArray,$connection);
$existingAttributes = getAttributeDataForBrick("scraper",$connection);


$data = false;
$data["name"] = "Refan";
$data["urls"] = "http://www.refan.dk/";
 */

/* $id = saveBrickData("scraper",$data,false,false,$connection);	
$scrapers = getBrickData("scraper",false,"*",false,false,$connection);
...
<script type="text/javascript">
	...
	function getScraperOptions (selected) {
		...
		$.ajax({
			type: "POST",
			url: "/system/includes/pages/import/ajax/ajax.scraper.php?v="+Math.random(),
			dataType: 'json',
			data: {
	...
	$(document).ready(function () {
		...
		$(".create-collection").click(function(){
			...
			$.ajax({
				type: "POST",
				url: "/system/includes/pages/import/ajax/json.php?v="+Math.random(),
				data: {
		...
		$(".import-csv").click(function(){
			...
			*/
			...
			$.ajax({
				type: "POST",
				url: "/system/includes/pages/import/ajax/ajax.json.php?v="+Math.random(),
				//contentType: "application/json; charset=utf-8",
				data: {
					mode: "getCSV"	,
					params: sc,
		...
		$('.import-scraper').click(function () {
			...
			if (urls.length && !error) {
				...
				$.ajax({
					type: "POST",
					url: "/system/includes/pages/import/ajax/ajax.output.php?v="+Math.random(),
					data: {
						urls: urls					
					},
					dataType: 'json',
