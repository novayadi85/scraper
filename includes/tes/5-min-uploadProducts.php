...
function tag_update($product , $customerId){
	return true;
}
function tag_update1($product , $customerId){
	global $searchobject, $connection , $customer  , $productLists , $productsToScrapes ,$allproducts;
...
function send2Shopify($id , $type = "product" , $customerId = false){
	global $searchobject, $connection , $customer , $productLists , $productsToScrapes ,$allproducts;
						...
						if(sizeof($params)){
							...
							*/
							...
							if(isset($products[strtolower($value["handle"])])){
								...
								if(isset($response["product"]["handle"]))
								tag_update($response["product"] , $customerId);
								curl_close ($ch);
							}
...
$quotes = array_slice($quoteProducts, 0, 400);

$response = array();

function send2Shopify2($id , $type = "product" , $customerId = null){
	$storeKey = getAPIStore($customerId);
	print $storeKey . "<br>";
} 
...
if(sizeof($quotes)){
	foreach($quotes as $quote){
		$response[$quote["id"]] = send2Shopify($quote["id"] , "product", $quote["parentid"]);
	}
} 
