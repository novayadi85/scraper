...
<script src="http://jeppekjaersgaard.dk/system/resources/assets/global/plugins/jquery.min.js" type="text/javascript"></script>
<?
...
file_put_contents($_SERVER["DOCUMENT_ROOT"].'/system/logs/job.log', "Works");
$searchobject = false;
$searchobject[] = array(
...
$productsToScrapes = getBrickData("productsToScrape",false,array("link"),false,$searchobject,$connection);
$urls = array_slice($productsToScrapes, 0, 10);
...
$(document).ready(function () {
	...
	function getData(){
		$.ajax({
			type: "POST",
			url: "//jeppekjaersgaard.dk/system/includes/pages/Scraper/import/ajax/ajax.scraper.php?v="+Math.random(),
			dataType: 'json',
			data: {
	...
	function doitNow() {
		...
		$.ajax({
			type: "POST",
			url: "//jeppekjaersgaard.dk/system/includes/pages/Scraper/import/ajax/ajax.output.php?v="+Math.random(),
			dataType: 'json',
			data: {
				urls: ids
			}, 
			success : function(objs){
				...
				if (obj.length) {
					...
					$.ajax({
						type: "POST",
						url: "//jeppekjaersgaard.dk/system/includes/pages/Scraper/import/ajax/ajax.json.php?v="+Math.random(),
